...
Given('the buzz user is on the buzz module page', async function (this: CustomWorld) {
  ...
  await this.page.goto(`${baseUrl}/web/index.php/buzz/viewBuzz`);
});

When('the buzz user performs the {string} action', async function (this: CustomWorld, action: string) {
...
Then('the buzz result is visible', async function (this: CustomWorld) {
  await expect(this.page.locator('body')).toBeVisible();
});
